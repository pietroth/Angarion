    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        if (typeElement == null) {
            processingEnv.getMessager()
                    .printError("br.angarion.dev.api.communication.Type was not found in the classpath");
        }
        ...
        for (Element el : roundEnv.getElementsAnnotatedWith(TypeConfiguration.class)) {
            ...
            String familyName = familyConfiguration != null ? familyConfiguration.value()
            ...
            for (Element enclosed : el.getEnclosedElements()) {
                if (enclosed.getKind().isField() && enclosed.getAnnotation(DataField.class) != null) {
                     ...
                     */
                    dataLayoutBuilder.addField(
                            FieldSpec.builder(TypeName.get(enclosed.asType()), enclosed.getSimpleName().toString())
        ...
        glossary.snapshot().forEach((key, value) -> {
            processingEnv.getMessager().printMessage(Diagnostic.Kind.NOTE,
                    "Family name: " + key + "; FamilyId: " + value.id() + ". " + value.types());
        });
